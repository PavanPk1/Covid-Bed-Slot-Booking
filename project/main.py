...
def signup():
    if request.method == "POST":
        ...
        new_user = db.engine.execute(
            f"INSERT INTO `user` (`srfid`,`email`,`password`) VALUES ('{srfid}','{email}','{encpassword}') ")
...
def test():
    try:
        a = Test.query.all()
        print(a)
        return f'COVIDDBMS DATABASE IS CONNECTED'
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        if (seat > 0 and check):
            ...
            flash("NO AVAILABLE BED ARE THERE FOR SELECED TYPE", "danger")
